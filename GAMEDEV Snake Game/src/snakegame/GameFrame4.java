...
public class GameFrame4 extends GameObject {
	...
	public void initResources() {
		...
		for (int x = MIN; x <= MAX; x++) {
			for (int y = MIN; y <= MAX; y++) {
				if (x == MIN || x == MAX || y == MIN || y == MAX || x == 9 && y>=0 && y<=9 || x >= 30 && x <= 38 && y == 9 || x >= 0 && x <= 9 && y == 30 || x == 30 && y >= 30 && y <= 38) {
		...
		collisionDeath = new SnakeDeath4();
		collisionDeath.setCollisionGroup(PLAYER, ENEMY);
		collisionFood = new SnakeEat4();
...
class SnakeEat4 extends BasicCollisionGroup {
	public SnakeEat4() {
	...
	public void collided(Sprite snake, Sprite food) {
		resetFood(food);
		GameFrame4.isEat = true;
	}

	public void resetFood(Sprite food) {
		int max = GameFrame4.MAX-1;
		int min = GameFrame4.MIN+1;
		int range = max-min+1;

		int newX = ((int)(Math.random() * range) + min);
		int newY = ((int)(Math.random() * range) + min);
		food.setX(GameFrame4.normalize((double) newX));
		food.setY(GameFrame4.normalize((double) newY));
	}
}

class SnakeDeath4 extends BasicCollisionGroup {
	public SnakeDeath4() {
		pixelPerfectCollision = false;
	}
