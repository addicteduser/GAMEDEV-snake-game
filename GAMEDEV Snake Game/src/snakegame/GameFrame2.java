...
public class GameFrame2 extends GameObject {
	...
	public void initResources() {
		...
		for (int x = MIN; x <= MAX; x++) {
			for (int y = MIN; y <= MAX; y++) {
				if (x == MIN || x == MAX || y == MIN || y == MAX || x >=11 && x<=29 && y>=19 && y<=20) {
		...
		collisionDeath = new SnakeDeath2();
		collisionDeath.setCollisionGroup(PLAYER, ENEMY);
		collisionFood = new SnakeEat2();
...
class SnakeEat2 extends BasicCollisionGroup {
	public SnakeEat2() {
	...
	public void collided(Sprite snake, Sprite food) {
		resetFood(food);
		GameFrame2.isEat = true;
	}

	public void resetFood(Sprite food) {
		int max = GameFrame2.MAX-1;
		int min = GameFrame2.MIN+1;
		int range = max-min+1;

		int newX = ((int)(Math.random() * range) + min);
		int newY = ((int)(Math.random() * range) + min);
		food.setX(GameFrame2.normalize((double) newX));
		food.setY(GameFrame2.normalize((double) newY));
	}
}

class SnakeDeath2 extends BasicCollisionGroup {
	public SnakeDeath2() {
		pixelPerfectCollision = false;
	}
